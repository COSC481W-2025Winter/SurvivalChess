...
export class ChessTiles {
	...
	pointerOver(i, j) {
		...
		if (this.boardState.isOccupied(i, j)) {
			...
			for (const tile of this.threats)
	...
	pointerOut(i, j) {
		...
		if (this.threats)
			for (const tile of this.threats) {
				const color = this.chessTiles[tile[0]][tile[1]].fillColor;
			...
			switch (this.boardState.getAlignment(i, j)) {
				case this.currentPlayer: // If it's the current player's piece
					...
					if (this.boardState.getRank(i, j) != KING && this.isChecked) {
						const coordinate = this.pieceCoordinates.getCoordinate(KING, this.currentPlayer);
					...
					for (const move of this.moves) this.highlightColor(move.xy, move.isEnemy ? LETHAL_COLOR : NON_LETHAL_COLOR);
					this.xy = [i, j];
					pointerOver = false;
					break;
	...
	spawnNextWave() {
		try {
			...
			const piecePriority = this.getPiecePriorityOrder();

			// Get the ordering of tiles to traverse through for new spawns
			const testSpawnLocations = this.getTilePriorityOrder();
			...
			while (currentBudget > 0 && !outOfSpawns) {
				...
				for (const pieceType of piecePriority) {
					const costOfType = this.getValueOfPiece(pieceType);
					...
					for (let pieceCount = 0; pieceCount < numberOfPieces; pieceCount++) {
						while (testSpawnIndex < testSpawnLocations.length) {
							const testSpawnLoc = testSpawnLocations[testSpawnIndex];
	...
	getTilePriorityOrder() {
		...
		const colOrder = Math.random() < 0.5 ? [3, 4, 5, 2, 1, 6, 7, 0] : [4, 3, 2, 5, 6, 1, 0, 7];
		const output = [];
	...
	getPiecePriorityOrder() {
		...
		const piecePriority = [QUEEN, PAWN, BISHOP, ROOK, KNIGHT];
	...
	clearBoard() {
		...
		if (this.moves) for (const move of this.moves) this.restoreColor(move.xy);
		this.xy = null;
		this.moves = null;
	}
	...
	isValidMove([col, row]) {
		...
		for (const move of this.moves) if (isSamePoint(move.xy, [col, row])) return true;
		return false;
	}
